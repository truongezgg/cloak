...
async function saveWorkspacePassword(root: string, password = 'secret123'): Promise<void> {
  await writeFile(join(root, '.cloak'), `PASSWORD=${password}\n`, 'utf8')
}

function createPrompts(overrides: Partial<ReturnType<typeof basePrompts>> = {}) {
...
it('prompts for local password on first run, rewrites .cloak, and encodes selected file', async () => {
  ...
  const prompts = createPrompts({ selectFile: vi.fn().mockResolvedValue(filePath) })
  ...
  expect(await readFile(join(root, '.cloak'), 'utf8')).toBe('PASSWORD=secret123\n')
  ...
  expect(prompts.confirmAction).toHaveBeenCalledWith('encode', canonicalPath, outputPath, false, foundContext)
...
it('uses existing local password without prompting', async () => {
  ...
  expect(prompts.selectFile).toHaveBeenCalledWith([{ name: '.env', path: await realpath(filePath) }], foundContext)
})
...
it('exits cleanly when local password prompt is cancelled', async () => {
  ...
  const prompts = createPrompts({ askLocalPassword: vi.fn().mockRejectedValue(new Error('User cancelled')) })

  await expect(runCloak({ cwd: root, prompts, directPath: filePath })).resolves.toBeUndefined()
  expect(prompts.selectFile).not.toHaveBeenCalled()
...
it('exits cleanly when file selection is cancelled', async () => {
  ...
  const prompts = createPrompts({ selectFile: vi.fn().mockRejectedValue(new Error('User cancelled')) })
...
it('propagates non-cancel selectFile failures and does not confirm', async () => {
  ...
  const prompts = createPrompts({ selectFile: vi.fn().mockRejectedValue(new Error('picker failed')) })

  await expect(runCloak({ cwd: root, prompts })).rejects.toThrow('picker failed')
  expect(prompts.confirmAction).not.toHaveBeenCalled()
...
it('rejects outside-root targets with message and no confirmation prompt', async () => {
  ...
  await expect(runCloak({ cwd: root, prompts, directPath: filePath })).rejects.toThrow(
    'File is outside the current directory',
  )
  expect(prompts.showMessage).toHaveBeenCalledWith(outsideRootMessage(canonicalOutside))
  expect(prompts.confirmAction).not.toHaveBeenCalled()
})
...
it('rejects reserved .cloak target before reading file contents', async () => {
  ...
  await expect(runCloak({ cwd: root, prompts, directPath: './.cloak' })).rejects.toThrow('Cannot read file')
...
it('uses decode flow for .cloak files and reports decode success', async () => {
  ...
  expect(prompts.confirmAction).toHaveBeenCalledWith('decode', canonicalPath, outputPath, false, foundContext)
...
it('fails decode after confirmation with wrong password and does not write output', async () => {
  ...
  await writeFile(filePath, await encodeTextFile('HELLO=world\n', 'different-password'), 'utf8')

  const prompts = createPrompts({ selectFile: vi.fn().mockResolvedValue(filePath) })
  const writer = vi.fn().mockResolvedValue(undefined)

  await expect(runCloak({ cwd: root, prompts, writeOutput: writer })).rejects.toThrow()
  expect(prompts.confirmAction).toHaveBeenCalledTimes(1)
  expect(writer).not.toHaveBeenCalled()
  await expect(access(join(root, '.env'))).rejects.toMatchObject({ code: 'ENOENT' })
})
...
it('shows overwrite warning when encode destination already exists', async () => {
  ...
  const prompts = createPrompts({ selectFile: vi.fn().mockResolvedValue(sourcePath) })
  ...
  const foundContext = await expectedPromptContext(root, true)
  expect(prompts.confirmAction).toHaveBeenCalledWith('encode', canonicalSource, outputPath, true, foundContext)
})
...
it('rejects .cloak input when marker is missing on first line', async () => {
  ...
  await expect(runCloak({ cwd: root, prompts })).rejects.toThrow('File is not protected by Cloak')
  expect(prompts.confirmAction).not.toHaveBeenCalled()
...
it('shows read failure and throws when direct path does not exist', async () => {
  ...
  await expect(runCloak({ cwd: root, prompts, directPath: './missing.env' })).rejects.toThrow('Cannot read file')
  expect(prompts.showMessage).toHaveBeenCalledWith('Cannot read file')
  expect(prompts.selectFile).not.toHaveBeenCalled()
  expect(prompts.confirmAction).not.toHaveBeenCalled()
})
...
it('shows read failure and throws when selected file is not valid UTF-8 text', async () => {
  ...
  await expect(runCloak({ cwd: root, prompts })).rejects.toThrow('Cannot read file')
  expect(prompts.showMessage).toHaveBeenCalledWith('Cannot read file')
  expect(prompts.confirmAction).not.toHaveBeenCalled()
})
...
it('shows Cannot write file when writing fails and leaves source unchanged', async () => {
  ...
  await expect(runCloak({ cwd: root, prompts, writeOutput: writer })).rejects.toThrow('Cannot write file')
  expect(await readFile(sourcePath, 'utf8')).toBe(original)
  await expect(access(outputPath)).rejects.toMatchObject({ code: 'ENOENT' })
  expect(prompts.showMessage).toHaveBeenCalledWith('Cannot write file')
})
